...
def ring_wanted(foreground, calibrating, has_slots, idle_pattern):
    """Whether Edgewise should own the LED ring right now.

    The badge's own pattern generator animates this ring smoothly at 30 fps for
    nothing, and it is what you see on boot. Holding the ring while we have
    nothing to show meant animating it ourselves *and* leaving the OS task
    spinning at 30 fps computing frames we discarded -- two systems doing one
    job, which is what made the result stutter.

    So: take the ring when there is something to say, and give it back when
    there is not. `idle_pattern` off means the user wants a dark ring rather
    than the badge's own colours, and a dark ring is something only we can
    provide -- the OS pattern would light it.
    """
    if not foreground:
        return False
    if calibrating or has_slots:
        return True
    return not idle_pattern
